...
def main():
    ...
    M = {'#':1, '.':0}
    S = [[M[s] for s in input()] for _ in range(N)]
    T = [[M[t] for t in input()] for _ in range(N)]
    S = shrink(S)
    T = shrink(T)
    for i in range(4):
        if S==T:
...
def transpose(S: List[List]) -> List[List]:
    ret = [[None for i in range(len(S))] for j in range(len(S[0]))]
