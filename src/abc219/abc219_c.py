...
def main():
    ...
    for i in range(N):
        S.append(input())
    org_X = 'abcdefghijklmnopqrstuvwxyz'
    char2i = {x:org_X[i] for i,x in enumerate(X)}
    i2char = {org_X[i]:x for i,x in enumerate(X)}

    new_S = []
    for s in S:
        new_s = ''.join([char2i[c] for c in s])
        new_S.append(new_s)
    
    new_S = sorted(new_S)

    for s in new_S:
        org_s = ''.join([i2char[c] for c in s])
        print(org_s)
