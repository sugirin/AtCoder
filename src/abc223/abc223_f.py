...
def main():
    ...
    for i, s in enumerate(S):
        if s =='(':
            lr_cnt.append(lr_cnt[i]+1)
        else:
            lr_cnt.append(lr_cnt[i]-1)

    diff0 = [0 for _ in range(N+1)]
    diffs = [diff0, diff0]
    for q, l, r in queries:
        if q==1:
            ...
            if sl==sr:
                continue
            if sl=='(':
                diff = [0 for _ in range(l)] + [-2 for _ in range(r-l)] + [0 for _ in range(N+1-r)]
            else:
                diff = [0 for _ in range(l)] + [2 for _ in range(r-l)] + [0 for _ in range(N+1-r)]
            diffs.append(diff)
            S_list = list(S)
            ...
            diffs_t = [sum(d) for d in zip(*diffs)]
            diffs = [diffs_t]
            prev = lr_cnt[l-1] + diffs_t[l-1]
            part = lr_cnt[l:r+1]
            for i, cnt in enumerate(part):
                if cnt - prev + diffs_t[l+i] < 0:
                    print('No')
                    break
            else:
                if prev != lr_cnt[r] + diffs_t[r]:
                    print('No')
                else:
                    print('Yes')
